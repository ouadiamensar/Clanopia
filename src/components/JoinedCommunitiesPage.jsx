...
const JoinedCommunities = () => {
  ...
  const fetchJoinedCommunities = async () => {
    ...
    try {
      ...
      console.log("📌 Fetching communities from:", userCommunitiesRef.path);
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        console.log("📌 Found community:", doc.id, data.communityName);
  ...
  useEffect(() => {
    ...
    console.log("👂 Listening to communities at:", userCommunitiesRef.path);
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setCommunities(communitiesData);
        setFilteredCommunities(communitiesData);
        console.log("✅ Updated with", communitiesData.length, "communities");
      },
      (error) => {
    ...
    return (
      <div
        ...
        <div className="flex flex-col items-center">
          ...
          <div className={getTextColor()}>{t("joinedCommunities.loading")}</div>
        </div>
      </div>
    );
  }
          ...
          <div className="flex-1 max-w-7xl w-full px-2 lg:px-5 py-2 lg:py-6">
            <div className="mb-8">
              <h1
                ...
                <RiCommunityLine className="inline mr-3 text-blue-500" />
                {t("joinedCommunities.title")}
              </h1>
              <p className={getSecondaryTextColor()}>
                {t("joinedCommunities.subtitle", { count: communities.length })}
              </p>
            </div>
            ...
            <div
              ...
              <div className="flex-1 min-w-[200px] relative">
                <input
                  type="text"
                  placeholder={t("joinedCommunities.searchPlaceholder")}
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className={`w-full pl-10 pr-4 py-2 ${getInputBg()} rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 ${getTextColor()} transition-colors duration-300`}
                />
            ...
            {filteredCommunities.length === 0 ? (
              <div
                ...
                <h3 className={`text-2xl font-bold ${getTextColor()} mb-2`}>
                  {searchQuery
                    ? t("joinedCommunities.noCommunitiesFound")
                    : t("joinedCommunities.noCommunitiesJoined")}
                </h3>
                <p className={`${getSecondaryTextColor()} mb-6`}>
                  {searchQuery
                    ? t("joinedCommunities.adjustSearch")
                    : t("joinedCommunities.joinFirst")}
                </p>
                <button
                  onClick={() => navigate("/Communities")}
                  className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-xl font-semibold transition-all duration-300 hover:scale-105">
                  <RiAddLine className="inline mr-2" />
                  {t("joinedCommunities.exploreCommunities")}
                </button>
              </div>
            ) : (
              <div
                    ...
                    <div className="p-4">
                      <p
                        ...
                        {community.communityData?.description ||
                          t("joinedCommunities.noDescription")}
                      </p>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-1">
                            ...
                            <span className={getSecondaryTextColor()}>
                              {formatNumber(
                                community.communityData?.membersCount || 0,
                              )}
                            </span>
                          </div>
                          <div className="flex items-center gap-1">
                      ...
                      <button
                        className="mt-3 w-full py-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-xl font-semibold transition-all duration-300 hover:scale-105 flex items-center justify-center gap-2"
                        onClick={(e) => {
                          e.stopPropagation();
                          navigateToCommunity(
                            community.id,
                        ...
                        {t("joinedCommunities.enterCommunity")}
                        <RiArrowRightLine />
                      </button>
                    </div>
                  </div>
